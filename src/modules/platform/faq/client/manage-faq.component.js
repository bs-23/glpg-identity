...
export default function ManageFaq() {
    ...
    const [topic] = useState(params.get('topic'));
    ...
    const urlChange = (orderBy, pageNo = 1, pageChange = false) => {
        let orderType = params.get('orderType');
        const page = pageNo ? pageNo : (params.get('page') ? params.get('page') : 1);

        if (!pageChange) orderType === 'asc' ? orderType = 'desc' : orderType = 'asc';

        const url = `?page=${page}`
            + (topic ? `&topic=${topic}` : '')
            + (orderBy ? `&orderBy=${orderBy}` : '')
            + (orderBy && orderType ? `&orderType=${orderType}` : '');
    ...
    const pageLeft = () => {
        if (faqData.metadata.page > 1) urlChange(params.get('orderBy'), faqData.metadata.page - 1, true);
    };

    const pageRight = () => {
        if (faqData.metadata.end !== faqData.metadata.total_items) urlChange(params.get('orderBy'), faqData.metadata.page + 1, true);
    };
                                    ...
                                    <Accordion className="cdp-table__responsive-accordion d-block d-sm-none">
                                        ...
                                        <Accordion.Collapse eventKey="0" className="cdp-table__responsive-accordion-body">
                                            <div className="cdp-bg-primary p-2 text-white">
                                                <span className={sort.value === 'question' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('question')}>Question<i className="icon icon-sort cdp-table__icon-sorting"></i></span>
                                                <span className={sort.value === 'answer' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('answer')}>Answer<i className="icon icon-sort cdp-table__icon-sorting"></i></span>
                                                <span className={sort.value === 'topics' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('topics')}>Topics<i className="icon icon-sort cdp-table__icon-sorting"></i></span>
                                                <span className={sort.value === 'created_by' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('created_by')}>Created By<i className="icon icon-sort cdp-table__icon-sorting"></i></span>
                                            </div>
                                        </Accordion.Collapse>
                                    </Accordion>
                                }
                                ...
                                <div className="d-flex justify-content-between align-items-center">
                                    <Dropdown className="ml-auto dropdown-customize">
                                        <Dropdown.Toggle variant className="cdp-btn-outline-primary dropdown-toggle btn d-flex align-items-center">
                                            <i className="icon icon-filter mr-2 mb-n1"></i> <span className="d-none d-sm-inline-block">{!topic ? 'Filter by Topics' : serviceTopics.find(x => x.slug === topic).title}</span>
                                        </Dropdown.Toggle>
                                        <Dropdown.Menu>
                            ...
                            <div className="table-responsive shadow-sm bg-white mb-3 cdp-table__responsive-wrapper">
                                <table className="table table-hover table-sm mb-0 cdp-table cdp-table__responsive">
                                    <thead className="cdp-bg-primary text-white cdp-table__header">
                                        <tr>
                                            <th width="25%"><span className={sort.value === 'question' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('question')}>Question<i className="icon icon-sort cdp-table__icon-sorting"></i></span></th>
                                            <th width="35%"><span className={sort.value === 'answer' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('answer')}>Answer<i className="icon icon-sort cdp-table__icon-sorting"></i></span></th>
                                            <th width="22%"><span className={sort.value === 'topics' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('topics')}>Topics<i className="icon icon-sort cdp-table__icon-sorting"></i></span></th>
                                            <th width="10%"><span className={sort.value === 'created_by' ? `cdp-table__col-sorting sorted ${sort.type.toLowerCase()}` : `cdp-table__col-sorting`} onClick={() => urlChange('created_by')}>Created By<i className="icon icon-sort cdp-table__icon-sorting"></i></span></th>
                                ...
                                {faqData.metadata.total_pages > 1 &&
                                    <div className="pagination justify-content-end align-items-center border-top p-3">
                                        <span className="cdp-text-primary font-weight-bold">{faqData.metadata.start + ' - ' + faqData.metadata.end}</span> <span className="text-muted pl-1 pr-2"> {' of ' + faqData.metadata.total_items}</span>
                                        ...
                                        <span className="pagination-btn" data-testid='Next' onClick={() => pageRight()} disabled={faqData.metadata.end === faqData.metadata.total_items}><i className="icon icon-arrow-down ml-2 next"></i></span>
                                    </div>
                                }
                            </div>
                        }
